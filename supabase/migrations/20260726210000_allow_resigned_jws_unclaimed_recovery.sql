...
create or replace function public.billing_claim_verified_unclaimed_app_store_entitlement(
  p_user_id uuid,
...
begin
  ...
  if p_summary_hash !~ '^[0-9a-f]{64}$' then
    raise exception using errcode = '22023', message = 'UNCLAIMED_RECOVERY_SUMMARY_HASH_INVALID';
  end if;
  ...
  -- signed_date identifies when Apple signed this JWS representation, not when
  -- the purchase happened. A later verified lookup may re-sign the same stable
  -- transaction facts. Only that timestamp and its derived summary hash may
  -- advance; every business fact remains immutable and fail-closed.
  if v_tx.original_transaction_id <> p_original_transaction_id
     ...
     or p_signed_date < v_tx.signed_date then
  ...
  if p_signed_date > v_tx.signed_date then
    update public.app_store_transactions set
      signed_date = p_signed_date,
      summary_hash = p_summary_hash
    where environment = p_environment and transaction_id = p_transaction_id;
  elsif p_summary_hash <> v_tx.summary_hash then
    raise exception using errcode = '23505', message = 'TRANSACTION_REPLAY_MISMATCH';
  end if;
...
comment on function public.billing_claim_verified_unclaimed_app_store_entitlement(
  ...
  timestamptz, timestamptz, timestamptz, timestamptz, integer, text, text, text, text,
  text, text, text, text, text
) is 'Atomically claims a verified tokenless Production entitlement, accepting only a later Apple JWS signing time when all stable transaction and current-entitlement evidence matches.';
