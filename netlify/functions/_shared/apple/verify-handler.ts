...
import {AppleServiceError, type AppleTransactionLookup, type AppleVerifier} from './types';
    ...
    try {
      ...
      if (!controls.appleLedgerWriteEnabled) {
        return json(200, {ok: true, mode: 'verification-only', persisted: false, membershipSynced: false,
          environment: summary.environment, status: summary.normalizedStatus});
      }
      const result = await (deps.persist ?? writeTransactionLedger)(
        deps.supabase, userId, environment, summary, !summary.appAccountToken,
      );
