...
test('binding, replay, ordering, sandbox and deletion invariants are explicit', () => {
  ...
  assert.match(sql, /p_current_source_signed_date > v_ent\.latest_signed_date/i);
  assert.match(sql, /p_current_source_signed_date = v_ent\.latest_signed_date/i);
  assert.match(sql, /CURRENT_STATE_AMBIGUOUS|current_state_ambiguous/i);
  ...
  assert.match(sql, /p_current_expires_at <= transaction_timestamp\(\)/i);
