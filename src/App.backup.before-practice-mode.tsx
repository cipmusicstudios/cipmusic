...
function PracticePanel({ 
  ...
  // Mock MIDI data for demonstration
  const mockMidi = React.useMemo(() => {
    const notes = ['C4', 'E4', 'G4', 'C5', 'G4', 'E4', 'C4', 'D4', 'F4', 'A4', 'D5', 'A4', 'F4', 'D4'];
  ...
  // Update active notes based on currentTime
  React.useEffect(() => {
    const active = mockMidi
      .filter(n => currentTime >= n.time && currentTime <= n.time + n.duration)
      .map(n => n.note);
    setActiveNotes(active);
  }, [currentTime, mockMidi]);
  ...
  return (
    <div className="fixed bottom-24 left-0 right-0 h-[40vh] z-40 animate-in slide-in-from-bottom duration-500">
      <div className="w-full h-full glass-effect border-t border-white/40 flex flex-col rounded-t-[40px] shadow-2xl overflow-hidden">
        
        {/* 1) Staff Section (50% height) */}
        <div className="h-[50%] relative flex flex-col justify-center overflow-hidden">
          {isLoading ? (
            <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-black/5 backdrop-blur-[2px]">
              <div className="w-8 h-8 border-2 border-[var(--color-mist-text)]/20 border-t-[var(--color-mist-text)] rounded-full animate-spin"></div>
              <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-[var(--color-mist-text)]/40">Loading MIDI Data...</span>
            </div>
          ) : (
            <>
              <div className="absolute inset-0 pointer-events-none bg-gradient-to-r from-[#c5beb5] via-transparent to-[#c5beb5] z-10 opacity-10"></div>
              
              {/* Staff Lines */}
              <div className="flex flex-col gap-[6px] px-4 opacity-15">
                {[1, 2, 3, 4, 5].map(i => <div key={i} className="h-[1px] w-full bg-[var(--color-mist-text)]"></div>)}
              ...
              <div className="absolute inset-0 flex items-center pointer-events-none overflow-hidden">
                <div 
                  className="flex relative h-full transition-transform duration-100 ease-linear"
                  style={{ transform: `translateX(calc(50% - ${currentTime * 100}px))` }}
                >
                  {mockMidi.map((n, i) => (
                    <div 
                      key={i}
                      className={`absolute h-3 rounded-full rotate-[-20deg] transition-all duration-300 ${activeNotes.includes(n.note) ? 'bg-[#c5a880] shadow-[0_0_12px_rgba(197,168,128,0.8)] scale-110' : 'bg-[var(--color-mist-text)]/40'}`}
                      style={{ 
                        left: `${n.time * 100}px`,
                        width: `${n.duration * 100}px`,
                        top: `${50 + (whiteKeys.indexOf(n.note.replace('#', '')) - 25) * 4}%`
                      }}
                    ></div>
                  ))}
                </div>
        ...
        <div className="h-[40%] px-2 pb-1 flex relative select-none">
          <div className="flex w-full h-full relative">
            {whiteKeys.map((note, i) => {
              ...
              const hasActiveBlack = activeNotes.includes(note + '#');

              return (
                <div 
                  key={note} 
                  className={`flex-1 border-r border-white/10 last:border-0 relative flex flex-col justify-end items-center pb-1 transition-all duration-200 ${isActive ? 'bg-[#c5a880]/40 shadow-[inset_0_-10px_20px_rgba(197,168,128,0.3)]' : note === 'C4' ? 'bg-[#c5a880]/10' : 'bg-[#c5beb5]/70'}`}
                  style={{ borderRadius: '0 0 2px 2px' }}
                >
                  {isC && (
                    <span className="text-[8px] font-bold text-[var(--color-mist-text)]/40 mb-1">{note}</span>
                  )}
                  
                  {/* Black Key Positioning */}
                  {hasBlackRight(note) && note !== 'C8' && (
                    <div 
                      className={`absolute top-0 right-0 w-[65%] h-[60%] rounded-b-sm z-30 translate-x-1/2 shadow-md border-x border-b border-white/10 transition-all duration-200 ${hasActiveBlack ? 'bg-[#c5a880] shadow-[0_0_10px_rgba(197,168,128,0.5)]' : 'bg-[var(--color-mist-text)]/90 hover:bg-[var(--color-mist-text)]'}`}
                    ></div>
                  )}
                </div>
              );
        ...
        <div className="h-[10%] min-h-[40px] border-t border-white/10 flex items-center justify-between px-10 bg-white/10">
          <div className="flex items-center gap-8">
            <button className="flex items-center gap-2 text-[var(--color-mist-text)]/60 hover:text-[var(--color-mist-text)] transition-colors">
              <Repeat className="w-3.5 h-3.5" />
              <span className="text-[10px] font-bold uppercase tracking-wider">Loop</span>
            </button>
            <div className="flex items-center gap-1 bg-white/10 p-0.5 rounded-lg border border-white/20">
              {[0.5, 0.75, 1].map(s => (
                <button 
                  key={s} 
                  onClick={() => setPlaybackRate(s)}
                  className={`px-2.5 py-1 rounded-md text-[10px] font-bold transition-all ${playbackRate === s ? 'bg-white/20 text-[var(--color-mist-text)] shadow-sm' : 'text-[var(--color-mist-text)]/40 hover:text-[var(--color-mist-text)]/60'}`}
                >
                  {s}x
                </button>
              ))}
          ...
          <button 
            onClick={onClose}
            className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 text-[var(--color-mist-text)] hover:bg-white/30 border border-white/20 transition-colors"
          >
